...
======================
Cassandra ODM is a lightweight model layer and CQL generator written in javascript that uses the official [datastax/nodejs-driver](https://github.com/datastax/nodejs-driver) project. The ODM supports data modeling, syncing tables, standard CRUD operations, Validations, Hooks, Plugins, Lightweight Transactions, Time to Live queries, batching, streaming queries, timing, and debugging queries (with 'pretty debugging' for supported terminals).
...
```javascript
    ...
    var config = {keyspace: "testkeyspace", hosts: ["127.0.0.1:9042"]};
    cassandra.connect(config);

    var CatSchema = new cassandra.Schema({name: String});
    var Cat = cassandra.model('Cat', CatSchema);

    cassandra.syncTables(config, function(err, results) {
...
Modeling
---------
Modeling is the process of defining your Schemas. Although Cassandra is a NoSQL database, it is required to make a column family with a primary key. ODM makes this process easier by helping you organize your code by defining Schemas in a single location (for easy reference). If you do not specify a primary key, ODM will automatically generate an 'id' key for you. Modeling also allows you to perform validations and apply pre and post hooks to your models. Finally, ODM will actually sync your tables forward to make rapid development easier ( tables and fields that don't exist in Cassandra will be created. ODM does not delete tables or fields as this could lead to data loss. ODM can warn you if there are unused fields though, see the "Sync" section for more information).

```javascript

    var cassandra = require('cassandra-odm'),
        Schema = cassandra.Schema; //Require ODM module

    var config = {keyspace: "testkeyspace", hosts: ["127.0.0.1:9042"]};
    cassandra.connect(config); //Connect to local cassandra server
    ...
    //Registers the schemas with cassandra
    var User = cassandra.model('User', UserSchema);
    var Blog = cassandra.model('Blog', BlogSchema);
    ...
    cassandra.syncTables(config, syncOptions, function (err, results) {
        console.log(err);
        ...
        newUser.save({debug: true, prettyDebug: true}, function (err, results) {
            ...
            //Note that for types other than arrays and maps, ODM tracks changes for saving, however, since blogs is an array, we need to mark it as modified
            //Also note that after running .save(), newBlog has a generated field called 'id'. This only occurs if ODM created the primary key for us (see "Primary Keys" for more info).
            ...
            cassandra.batch([firstQuery, secondQuery], {consistency: cassandra.consistencies.quorum, debug: true, prettyDebug: true}, function (err, results) {
                if (err) console.log(err);
...
The above example shows a lot of code, but is relatively simple to understand (particularly if you've used Mongoose). First, we connect to the Cassandra server. Then, we create some schemas with ODM (along with a validator for username and a post-save hook on users). After we register the Schemas with ODM, we sync the tables to make sure that Cassandra knows that they exist (see "Sync" for more information on this and the limitations of syncing). Also note that we haven't provided a primary key in any of our schemas. In general, its good practice to explicitly define a primary key in Cassandra (and Cassandra requires it actually). ODM takes care of this requirement by generating a field called 'id' if we don't specify a primary key. After we call the sync tables function, we can now create users and blogs in our database. First, we create a new user and save it. Then we create a new blog and store the query to be called with some other updates. Once we've done our updates locally, we gather the queries and send them in a batch to our Cassandra server using the cassandra.batch command to create our blog post and update our user. Finally, we close the connection when we're done.

NOTE: All fields should be lowercase. This is due to Cassandra's columns being lowercase (and currently ODM doesn't automatically covert these for you in javascript).
...
```javascript
    ...
    cassandra.model('Fish', FishSchema);
...
Note: ODM internally stores a flag to know when you've modified fields - for arrays and maps, you must specified that a field has been modified using the Model.markModified('fieldName'); method though (see 'Modeling' for an example)
...
```javascript
    ...
    fishee2.save(function(err) {
        ...
        fishee2.save(function(err) {
            //fishee2 has now been renamed (ODM internally stores a flag to know when you've modified fields - for arrays and maps, you must specified that a field has been modified using the fishee2.markModified('fieldName'); method though (see 'Modeling' for an example).
        });
    });
...
Types
----------
ODM supports the following types. Note that arrays and Maps must have defined types.

String
Number (can specify Int by using cassandra.types.Int, Double by cassandra.types.Double, or Long by cassandra.types.Long) - default is Int if you use Number
Date (a timestamp)
ObjectId (specified by cassandra.types.ObjectId or cassandra.types.uuid) - this is a uuid v4
...
NOTE: Currently ODM generates a table name by using a lowercase pluralized version of the Model name you specify (ie 'User' becomes 'users').
...
```javascript
        ...
        //Registers the schemas with ODM
        var User = cassandra.model('User', UserSchema);
        var Blog = cassandra.model('Blog', BlogSchema);
        
        var syncOptions = {debug: true, prettyDebug: true, warning: true};
        cassandra.syncTables(config, syncOptions, function (err) {
            console.log(err);
        });

```

When writing an application, the general idea is that you preload all your schemas into ODM, then sync your models once before running your other code (ie before starting an express application or web server). This will give you access to all your models whenever you need them by using the cassandra.model('ModelName') function. This is similar to how Mongoose handles preloading schemas.

A final thing to note is that you can specify Keyspace replication strategy's in your ODM config (if you let ODM create your keyspaces for you - you can do this yourself through cqlsh, but ODM can automate the process as well). See "Keyspace Replication Strategy and Production Notes" for more information.
...
Primary Keys
----------
Cassandra requires a primary key for all column families. This means that you would need to define a primary key whenever creating a table. ODM relaxes that restriction slightly by allowing you to define Schemas without primary keys. However, what ODM does internally is create an 'id' field on your Schema and adds a pre-save hook to generate an id for all new models (see "Hooks" for how you can do the same with your own fields). ODM then marks this 'id' field as your primary key. This is important to note because primary keys cannot be modified using an ALTER TABLE command. In Cassandra, once your primary key has been set for a table, you're not allowed to modify it. If you want sophisticated primary keys (like composite primary keys), you need to design your Data Model appropriately from the beginning and make sure that you define it correctly in your Schema.
...
The examples below show how a primary key can be explicitly defined on a field, how a composite primary key can be defined, and how to allow ODM to generate a primary key for you:
...
```javascript
    ...
    //ODM defines 'id' field for you - Note that in this case 'dog_id' is NOT the primary key, 'id' is (and 'id' is a uuid v4 type)
    //ODM will also add a presave function that adds the 'id' field for you before you save a new model.
...
Secondary Indexing
----------
ODM supports secondary indexes on fields with the following option {index: true}. See the example below:
...
Validations
----------
Validations are a core part of ODM's Object Data Model. Validations allow you to easily reject inserts and updates across all your models in javascript without ever hitting your database. ODM comes with internal support for a "required" validation and also allows you to validate any field with a custom function.
...
ODM supports pre-save and pre-remove hooks for its models. It also supports post-init, post-validate, post-save, and post-remove hooks. The example below shows all of these being used.
...
```javascript
    ...
    cassandra.model('Trick', TrickSchema);

```

Note that hooks are only called on ODM object instances, not when performing Model.update or Model.remove (because those are direct database calls that don't generate any ODM instances).
...
ODM supports lightweight transactions for saving new data via the {if_not_exists: Boolean} option.
...
Time to Live (TTL)
----------
ODM supports specifying a TTL when inserting data via the {ttl: Number} option, where Number is the time in seconds. Also see "Sessions".

```javascript

    var Post = cassandra.model('Post');
...
Limit & Sorting
----------
ODM can limit your queries based on options or by chaining queries. See the examples below:
...
Batching
----------
ODM can batch queries together to run at once. This is done by not specifying a callback to an insert or delete function and passing an array of queries to cassandra.batch(). See the example below:
...
```javascript
    ...
    cassandra.batch([query_1, query_2], batchOptions, function(err) {
        //Handle errors, etc.
        if(err) console.log(err);
    });
...
Execute Prepared
----------
ODM can execute prepared queries by passing in a "prepared" option when calling exec (either through a callback or through Query.exec() directly). See the examples below:
...
Streaming
----------
ODM supports streaming results via a Query.stream(options, callback) method. This returns a readable stream (can view documentation for node-cassandra-cql streams as well). See the example below:
...
----------
ODM supports timing and debugging capabilities (including a prettyDebug mode which prints using colored text to a supported terminal and is far more human readable than standard debugging). These options are supported on almost all ODM queries (timing is not supported on sync tables because it should really only be called once after preloading all your schemas). To use the options, simply pass the following object to a query as part of its options:
...
----------
Client connections are handled by node-cassandra-cql. ODM encapsulates a connection internally, but you can also use the node-cassandra-cql connection directly for CQL queries:

```javascript

    var cassandra = require('cassandra-odm');
    var connection = cassandra.connect({keyspace: "mykeyspace", hosts: ["127.0.0.1:9042"]});
...
----------
There are a few queries that can be efficiently executed if you use on-disk sorting of columns when creating tables. ODM allows you to specify options for creating tables when defining your schemas. See "Data Modeling Notes" for more information on efficient client queries. See [Using Clustering Order](http://www.datastax.com/documentation/cql/3.1/cql/cql_reference/refClstrOrdr.html) for Clustering Order option. ODM only supports the clustering order option at the moment (adding all table properties is on the roadmap, you can modify them manually currently using CQL's ALTER TABLE command).
...
var EventSchema = new Schema({
        event_type: String,
        insertion_time: cassandra.types.Timestamp,
        event: cassandra.types.Blob
    },
    {
...
Note: 'npm test' creates a keyspace "cassandratest" on your local Cassandra server then deletes it when done.
